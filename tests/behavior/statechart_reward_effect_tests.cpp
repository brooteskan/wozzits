...
// `toward` (true = the 0 branch) became `branch` (true = the 1 branch).
//
...
// `branch: true` names the 1 branch, and reaches the runner as flag 1.
TEST(StatechartRewardEffect, BranchTrueSelectsTheOneBranch)
{
    sc::Chart chart;
    std::string error;
    ASSERT_TRUE(sc::parse_chart(
        chart_with_reward_action(
            R"({"kind":"reward","agent":"a","q":2,"branch":true,)"
...
// `branch: false` names the 0 branch, and an absent `branch` defaults to it.
TEST(StatechartRewardEffect, BranchFalseAndAbsentSelectTheZeroBranch)
{
    for (const char* action : {
             R"({"kind":"reward","agent":"a","q":0,"branch":false,)"
             R"("strength":{"const":0.5}})",
...
TEST(StatechartRewardEffect, LegacyTowardKeyIsRefusedWithAnActionableMessage)
{
    for (const char* legacy : {
        ...
        EXPECT_NE(error.find("branch"), std::string::npos) << error;
    }
}
