...
namespace wz::app
{
        ...
        const wz::input::InputState& input, float dt, bool drive_camera)
    {
        ...
        frame_profiler_.end_frame(
            behavior_frame_index_, static_cast<double>(dt) * 1000.0, sim_ms,
            document_.nodes().size(), ctx_.logger);
    }
    ...
    {
        frame_profiler_.record_rematerialize(caller);
    ...
    {
        // The profiler owns the capture buffer + CSV flush; the app just wires it
        // to the asset library (for the flush) and the logger (#219 avenue 5).
        frame_profiler_.set_enabled(enabled, ctx_.assets.get(), ctx_.logger);
    }
    ...
    WozzitsApp_v1::SceneSavePreparation WozzitsApp_v1::prepare_scene_save()
    {
        ...
        frame_profiler_.flush_csv(ctx_.assets.get(), ctx_.logger);
