// src/gpu/dx12/dx12_hdr_image_texture.cpp
...
namespace wz::gpu::dx12::internal
{
        ...
        const wz::engine::assets::HDRImageData& image)
    {
        ...
        rgba.resize(static_cast<size_t>(pixel_count) * 4u, 1.0f);
        for (uint32_t y = 0; y < image.height; ++y) {
            for (uint32_t x = 0; x < image.width; ++x) {
                const size_t src =
                    (static_cast<size_t>(y) * image.width + x) * channels;
                const size_t dst =
                ...
                rgba[dst + 3] = channels >= 4 ? image.pixels[src + 3] : 1.0f;
            }
        }
        ...
        const uint64_t src_row_bytes =
            static_cast<uint64_t>(image.width) * 4u * sizeof(float);
        for (uint32_t y = 0; y < image.height; ++y) {
            std::memcpy(
