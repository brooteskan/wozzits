// src/gpu/dx12/dx12_vector_field_texture.cpp
...
namespace wz::gpu::dx12::internal
{
        ...
        const wz::engine::assets::VectorFieldData& field)
    {
        ...
        if (!impl || !impl->device || !impl->queue)
            return INVALID_GPU_HANDLE;
        if (!impl->fence || !impl->fence_event)
            return INVALID_GPU_HANDLE;
        if (!impl->scalar_field_srv_heap)
            return INVALID_GPU_HANDLE;
        if (!field.valid() || field.depth != 1)
            return INVALID_GPU_HANDLE;
        if (field.format != wz::engine::assets::VectorFieldFormat::Float32)
            return INVALID_GPU_HANDLE;
        ...
        rgba.resize(static_cast<size_t>(sample_count) * 4u, 0.0f);

        // V0 visualizes the first vector channel. Multi-channel field blending
        // and named channel selection belong in the future field/sky material
        // policy rather than in this uploader.
        for (uint32_t y = 0; y < field.height; ++y) {
            for (uint32_t x = 0; x < field.width; ++x) {
                ...
                rgba[dst + 3] = component_count >= 4 ? rgba[dst + 3] : 1.0f;
            }
        }
        ...
        const uint64_t src_row_bytes =
            static_cast<uint64_t>(field.width) * 4u * sizeof(float);
        for (uint32_t y = 0; y < field.height; ++y) {
            std::memcpy(
        ...
        ID3D12GraphicsCommandList* upload_cmd = nullptr;
        hr = d3d->CreateCommandList(
            0,
            D3D12_COMMAND_LIST_TYPE_DIRECT,
            upload_allocator,
            nullptr,
            IID_PPV_ARGS(&upload_cmd));
        if (FAILED(hr)) {
        ...
        upload_cmd->CopyTextureRegion(&dst, 0, 0, 0, &src_loc, nullptr);
        ...
        ID3D12CommandList* lists[] = { upload_cmd };
